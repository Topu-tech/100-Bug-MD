const { default: makeWASocket, DisconnectReason, fetchLatestBaileysVersion, useMultiFileAuthState, makeCacheableSignalKeyStore } = require('@whiskeysockets/baileys');
...
// Auth folder const authFolder = path.join(__dirname, 'auth');

// Write base64 session if not already written if (config.SESSION_ID) { try { const sessionData = config.SESSION_ID.replace(/^ALONE-MD;;;=>/, ''); const decoded = Buffer.from(sessionData, 'base64').toString('utf-8');

fs.mkdirSync(authFolder, { recursive: true });
fs.writeFileSync(path.join(authFolder, 'creds.json'), decoded, 'utf-8');
console.log('✅ Session decoded and written.');

} catch (err) { console.error('❌ Failed to decode SESSION_ID:', err); process.exit(1); } }
...
// Start the bot async function startBot() { const { state, saveCreds } = await useMultiFileAuthState(authFolder); const { version } = await fetchLatestBaileysVersion();
...
// Send presence update based on PRECENCE env variable
const precence = (config.PRECENCE || '').toLowerCase();
try {
  if (precence === 'available') {
    await sock.sendPresenceUpdate('available', from);
  } else if (precence === 'composing') {
    await sock.sendPresenceUpdate('composing', from);
  } else if (precence === 'recording') {
    await sock.sendPresenceUpdate('recording', from);
  } else {
    await sock.sendPresenceUpdate('unavailable', from);
  }
  console.log(`📶 Presence updated: ${precence}`);
} catch (err) {
  console.error('⚠️ Failed to send presence:', err);
}
...
// Dummy HTTP server to keep Render alive http.createServer((req, res) => { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('🤖 WhatsApp bot is running.\n'); }).listen(process.env.PORT || 3000, () => { console.log('🌐 HTTP server listening to keep Render alive'); });
